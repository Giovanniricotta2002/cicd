...
$ent = __("numerisation") . " -> " . __( "suivi dossier" ) . " -> " . __( "num_dossier_a_numeriser" );
if (isset ( $idz ) && trim ( $idz ) != '') {
    $ent .= "&nbsp;" . strtoupper ( $idz ) . "&nbsp;";
}

// Titre de l'onglet
$tab_title =  sprintf('%s %s', __("suivi des"), __("num_dossier_a_numeriser"));

// Restriction aux num_dossiers à numériser
if ( $selection == "" ) {
    $selection = " WHERE";
} else {
    $selection .= " AND";
}
$selection .= " datenum IS NULL AND num_dossier.num_bordereau IS NOT NULL";

// TRI 
$tri="ORDER BY envoi ASC, num_commande ASC";
