...
$ent = __("numerisation") . " -> " . __( "suivi dossier" ) . " -> " . __( "num_dossier_traite" );
if (isset ( $idz ) && trim ( $idz ) != '') {
    $ent .= "&nbsp;" . strtoupper ( $idz ) . "&nbsp;";
}

// Titre de l'onglet
$tab_title =  sprintf('%s %s', __("suivi des"), __("num_dossier_traite"));

// Restriction aux dossiers à attribuer
if ( $selection == "" ) {
    $selection = " WHERE";
} else {
    $selection .= " AND";
}
$selection .= " num_bordereau.num_bordereau IS NOT NULL AND datenum IS NOT NULL  ";

// TRI 
$tri="ORDER BY datenum ASC, num_commande ASC";
