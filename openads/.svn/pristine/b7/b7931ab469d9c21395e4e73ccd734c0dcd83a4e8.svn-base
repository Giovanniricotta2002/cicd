<?php
/**
 * LISTING - Les Audiances
 *
 * Le listing 'Mes Clôtures' permet d'afficher les même dossiers que 
 * le widget 'Mes Clôtures' sans la limite de 5 enregistrements. Aucune 
...
//
$conf = $om_widget->get_config_dossier_contentieux_clotures($params);

/**
 * Configuration du listing
 */
// Titre de la page
$ent = _("contentieux")." -> "._("Mes Clôtures");
//
...
$tab_actions['left']['consulter'] = array(
    'lien' => ''.OM_ROUTE_FORM.'&obj=dossier_contentieux_tous_recours&amp;action=3&amp;idx=',
    'id' => '',
...
$selection = $om_widget->add_filter_division_to_query($selection, $conf["arguments"]["filtre"]);
// ORDER BY
$tri = sprintf("
    ORDER BY
        %s
