...
class om_reqmo extends reqmo {
    ...
    public function view_reqmo() {
        ...
        if ($usecase == "reqmo-list") {
            ...
            // Spécifique aux requêtes mémorisées de la table bible et
            // evenement qui nécessite l'utilisation des caractères "[" et "]"
            // pour la manipulation de tableau "ARRAY"
            $reqmo['sql']=str_replace('|stab|', '[', $reqmo['sql']);
            $reqmo['sql']=str_replace('|etab|', ']', $reqmo['sql']);
