...
$reqmo['libelle']=_("Liste simplifiee des dossiers deposes");
...
//
$reqmo['tri']= array('dossier.date_depot');
...
WHERE dossier.om_collectivite IN (<idx_collectivite>)
    AND etat.statut = 'encours' AND 
    dossier_autorisation_type.dossier_autorisation_type = '[dossier_autorisation_type]' AND 
    dossier.date_depot >=  '[date_depot_debut]' AND
    dossier.date_depot <=  '[date_depot_fin]' AND 
    dossier_instruction_type.code = 'P'".
    $sqlFiltreSD.
